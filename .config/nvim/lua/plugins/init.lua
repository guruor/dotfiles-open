...
local default_plugins = {
  ...
  -- Git stuff
  {
    ...
    dependencies = {
      -- "ruifm/gitlinker.nvim", -- Similar functionality like fugitive GBrowse
      "linrongbin16/gitlinker.nvim", -- Fork for ruifm/gitlinker.nvim
    },
  },
  {
    "linrongbin16/gitlinker.nvim",
		lazy = false,
    init = function()
      -- load gitlinker only when a git file is opened
      vim.api.nvim_create_autocmd({ "BufRead" }, {
        group = vim.api.nvim_create_augroup("GitLinkerLazyLoad", { clear = true }),
        callback = function()
          vim.fn.system("git -C " .. '"' .. vim.fn.expand "%:p:h" .. '"' .. " rev-parse")
          if vim.v.shell_error == 0 then
            vim.api.nvim_del_augroup_by_name "GitLinkerLazyLoad"
            vim.schedule(function()
              local opts = require("plugins.configs.misc").gitlinker()
