...
M.rename = function()
    ...
    vim.api.nvim_buf_set_var(bufnr, "rename_vars",
                             {current_name = current_name, params = vim.lsp.util.make_position_params(), orig_bufnr = orig_bufnr})
...
M.callback = function()
    ...
    vim.cmd [[stopinsert]]
    vim.cmd [[bd!]]
    if #new_name == 0 or new_name == vars.current_name then return end
